"""
Takes in a string and search that string(case insensitive) inside the webpage source code if found a match then returns the link for it
"""
import sys
import argparse
import requests
...
def crawl(url, start_domain, search_string,depth,max_depth):
    if max_depth!=None :
        if depth>max_depth :
            return 
    """
    ...
    for link in links:
        if "href" in link.attrs:
            ...
            process_url(next_url, start_domain, search_string,depth,max_depth)
...
def process_url(url, start_domain, search_string,depth,max_depth):
    """
    ...
    if domain == start_domain:
        if url not in visited_links:
            if max_depth is None or depth <= max_depth:
                ...
                crawl(url, start_domain, search_string,depth+1,max_depth)
    else:
        if depth <=max_depth or max_depth is None :
...
if __name__ == "__main__":
    ...
    crawl(start_url, start_domain, search_string,1,threshold)
